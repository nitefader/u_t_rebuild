...
| D7 | TOCTOU fix uses single `with conn` block + WAL, not optimistic version stamps | WAL gives reader-writer concurrency without blocking. Single connection per evaluation gives transactional consistency. Optimistic version stamps are a heavier refactor for the same outcome. **T-6 implementation clarification (2026-04-30):** the single-conn boundary lives on `SQLiteRuntimeStore.load_governor_policy_inputs(account_id, horizon)`, which reads `account_risk_configs` and `account_risk_plan_map ⨝ risk_plan_versions` inside ONE `with self._connect()` block. To make the boundary realizable, `GovernorPolicyResolver` was refactored from two independent lookup callbacks to one composite `get_policy_inputs` callback so both halves of the snapshot come from one call. Last-writer-wins outside the evaluation window is the intended behavior — there is no `risk_plan_map_concurrent_modification` rejection rule. |
