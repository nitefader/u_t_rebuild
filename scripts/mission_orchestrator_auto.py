...
Operator writes a mission in operator_mission.txt. Claude plans and reviews.
Codex implements. The operator is paged only when Claude cannot safely decide.

Two-way operator question flow:
  1. Claude emits OPERATOR_QUESTION.
  2. This script writes scripts/operator_questions.txt and HALT.flag.
  3. Operator edits operator_questions.txt and adds an ANSWER section.
  4. Operator reruns the loop.
  5. This script clears HALT.flag, feeds the answer back to Claude, and continues.
"""
...
def operator_answer_present() -> bool:
    ...
    after_answer = text[match.start():].strip()
    return len(after_answer) > len("ANSWER:")
...
def write_operator_question(cycle: int, reason: str, payload: str) -> None:
...
def run_one_cycle(
    ...
    if parsed["directive"] == "OPERATOR_QUESTION":
        write_operator_question(cycle, parsed["reason"], parsed["payload"])
        halt(
            state,
