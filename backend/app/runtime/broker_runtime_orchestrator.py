...
from backend.app.features import FeatureCache, IncrementalFeatureEngine, NormalizedBar, ResolvedProgramComponents
...
class BrokerRuntimeOrchestrator:
    """Continuous BROKER_PAPER runtime coordinator.
    ...
    def load_active_broker_paper_deployments(self) -> tuple[BrokerRuntimeDeployment, ...]:
        if self._runtime_store is not None and hasattr(self._runtime_store, "list_active_broker_paper_deployments"):
            loaded = tuple(self._runtime_store.list_active_broker_paper_deployments())
            self._deployments.update({entry.deployment.deployment_id: entry for entry in loaded})
        return tuple(entry for entry in self._deployments.values() if entry.active and entry.deployment.mode == TradingMode.BROKER_PAPER.value)
    ...
    def process_completed_bar(self, deployment_id: UUID, normalized_bar: NormalizedBar) -> PipelineResult | None:
        ...
        state = self._load_state(deployment_id).model_copy(
            update={
                ...
                "last_execution_intent_timestamp": result.execution_intents[-1].timestamp if result.execution_intents else self._load_state(deployment_id).last_execution_intent_timestamp,
    ...
    def _deployment_entry(self, deployment_id: UUID) -> BrokerRuntimeDeployment | None:
        if deployment_id not in self._deployments:
            self.load_active_broker_paper_deployments()
    ...
    def _preflight(self, entry: BrokerRuntimeDeployment, *, require_recovery: bool) -> BrokerRuntimeLoopStatus | None:
        ...
        if not entry.active or entry.deployment.mode != TradingMode.BROKER_PAPER.value:
            return self._block(deployment_id, "deployment_not_broker_paper_active")
        ...
        if account.mode != TradingMode.BROKER_PAPER:
            return self._block(deployment_id, "paper_runtime_rejected_non_paper_account")
