...
class V4ExpressionSignalSource:
    ...
    def evaluate(
        ...
        if contexts.evaluation_type == "logical_exit":
            ...
            position_context = contexts.position_contexts.get(symbol.upper())
            if position_context is None:
                position_context = PositionSignalContext(has_position=False)
            intents, diagnostics = evaluate_v4_logical_exits(
            ...
            return SignalEvaluationResult(
                decision="emitted" if intents else "no_signal",
