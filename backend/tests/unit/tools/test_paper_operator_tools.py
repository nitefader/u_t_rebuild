...
def test_paper_runtime_produces_events(monkeypatch, capsys) -> None:  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr(paper_runtime, "AlpacaBrokerAdapter", FakeRuntimeAdapter)

    code = paper_runtime.main(["--bars", "1"])
...
def test_paper_runtime_governor_blocks_when_required() -> None:
    components = paper_runtime._components(symbol="SPY", qty=1)
    deployment = paper_runtime.DeploymentContext(
        deployment_id=paper_runtime.DEFAULT_DEPLOYMENT_ID,
        program=components.program,
        mode="paper_runtime",
    )
    orchestrator = RuntimeOrchestrator(
        account_id=paper_runtime.DEFAULT_ACCOUNT_ID,
        deployment=deployment,
        components=components,
        governor=PortfolioGovernor(GovernorPolicy(global_kill_active=True)),
    )

    result = orchestrator.process_bar(paper_runtime._generated_completed_bars(symbol="SPY", count=1)[0])
...
def test_paper_runtime_order_flows_through_all_layers(monkeypatch, capsys) -> None:  # type: ignore[no-untyped-def]
    _paper_runtime_env(monkeypatch)
    FakeRuntimeAdapter.submitted_orders = []
    FakeRuntimeAdapter.market_is_open = True
    calls: list[str] = []
    ...
    monkeypatch.setattr(paper_runtime, "AlpacaBrokerAdapter", FakeRuntimeAdapter)
    monkeypatch.setattr(paper_runtime, "OrderManager", RecordingOrderManager)
    monkeypatch.setattr(paper_runtime, "BrokerSync", RecordingBrokerSync)

    code = paper_runtime.main(["--bars", "3"])
...
def test_paper_runtime_market_closed_exits_without_submit(monkeypatch, capsys) -> None:  # type: ignore[no-untyped-def]
    _paper_runtime_env(monkeypatch)
    FakeRuntimeAdapter.submitted_orders = []
    FakeRuntimeAdapter.calls = []
...
def test_paper_runtime_blocks_without_confirmation(monkeypatch, capsys) -> None:  # type: ignore[no-untyped-def]
    _paper_env(monkeypatch)
    FakeRuntimeAdapter.submitted_orders = []
    monkeypatch.setattr(paper_runtime, "AlpacaBrokerAdapter", FakeRuntimeAdapter)
