...
**Current implementation alignment:** 2026-04-26

**Latest product override:** `Simplified Runtime Architecture and Guiding Principles.md`
is the first authority for the V1 product model. Where this document still uses
the older Program-centric decomposition, apply the simplified model below.

Simplified V1 canonical model:

- `Strategy` is the saved, reusable trading logic and configuration.
- `Watchlist` is the saved symbol source.
- `Deployment` is a running Strategy publisher over selected Watchlists.
- `Deployment` emits neutral `SignalPlan`s.
- `Account` / `BrokerAccount` subscribes to Deployments and independently accepts, rejects, sizes, or ignores each SignalPlan.
- `Governor` / `PortfolioGovernor` protects the Account before any order reaches BrokerAdapter.
- `Order` and `Trade` are account-specific and link back to SignalPlan and Deployment.
- There is no user-facing `Program` entity in V1.
- If an Account accepts an opening SignalPlan, it must keep listening to the same Deployment signal stream for related close, reduce, scale-out, stop, target, runner, and logical-exit SignalPlans.
- Account-owned positions must be explainable from signal lineage, account decision trace, Governor decision, orders, fills, current risk, and related close/reduce SignalPlans.

The current codebase already treats these names as canonical:

- `TradingMode` in `backend/app/domain/trading_mode.py`
- `BrokerAccount` in `backend/app/broker_accounts/`
- `DeploymentContext` / deployment runtime state in `backend/app/runtime/`
- `MarketDataServiceRecord` as provider configuration
- `MarketDataPipeline` as reusable market-data stream identity
- `PortfolioGovernor`, `OrderManager`, `BrokerAdapter`, and `BrokerSync` as separate runtime boundaries

Any new implementation work should converge toward this document and the
mode contract in `docs/system_rebuild_outputs/MODE_NAMING_CONTRACT.md`.
...
The full data-and-execution spine is:

`Program -> Deployment -> Feature Planner -> Resolver -> MarketDataPipeline -> BarBuilder -> Feature Engine -> Signal Engine -> Strategy Controls -> Risk Profile -> Execution Style -> Portfolio Governor -> OrderManager -> BrokerAdapter -> BrokerSync`

Short rule:

`Features drive demand. Pipelines deliver data. Accounts isolate risk. Governors approve trades. BrokerSync writes broker truth.`
...
**Purpose:** A configured external market-data provider connection.

**Owns:**
- provider identity
- credential reference or credential absence
- validation status
...
- Yahoo historical-data service

### MarketDataPipeline

**Purpose:** A reusable market-data stream or fetch identity.

**Owns:**
- selected service link
- provider/feed/trading-mode identity tuple
- subscription fan-out
- market-data freshness
- normalized bar delivery to internal consumers

**Must not own:**
- broker account event streams
- order/fill reconciliation
- signal or feature computation
- portfolio approval

One paid market-data stream may feed many BrokerAccounts and many Deployments.
Do not open duplicate market-data streams per BrokerAccount.

### Broker Account Event Stream

**Purpose:** Account-scoped broker truth updates.
...
**Must not be modeled as:**
- a MarketDataService
- a MarketDataPipeline
- a shared cross-account stream

Broker account event streams are account-scoped and reconcile through BrokerSync.
...
| MarketDataServiceRecord | Provider config, credentials reference, validation, capabilities | Runtime authority, orders, broker truth, features |
| MarketDataPipeline | Shared data subscription/fetch identity, fan-out, freshness | Broker account events, execution, reconciliation |
| BrokerSync | Broker-derived truth writes and reconciliation | Signal generation, provider selection, simulated truth |
...
| `Paper Runtime` | `Broker Runtime - Paper` | Broker runtime using paper broker endpoints/accounts |
| `Live Runtime` | `Broker Runtime - Live` | Broker runtime using live funded broker accounts |
| `Services Center` | `Providers` | UI area for external provider configuration |
| `Market Data Service` | `MarketDataServiceRecord` | Provider configuration and validation record |
| `Data Stream` / `Market Data Stream` | `MarketDataPipeline` | Shared data stream/fetch identity |
| `Program Pause` | `Deployment Pause` | Runtime pause scoped to one running deployment |
| `Provider mode` | `provider_environment` or `broker credential environment` | Vendor/account environment, not Trading OS system mode |
| `Program` as user-facing V1 entity | `Strategy` + `Deployment` + `SignalPlan` | Program is not a core saved V1 product entity |
| `CandidateTradeIntent` for published runtime idea | `SignalPlan` | Neutral account-independent trade plan emitted by Deployment |
| `Deployment on BrokerAccount` | `Account subscribes to Deployment` | Deployment publishes; Account decides and executes |
| `Risk Profile` as required V1 table | `Account.risk_config_json` | Account owns enforceable risk config in V1 |
| `Execution Style` as required V1 table | `Strategy.config.execution` | Execution plan is config-driven in V1 |
| `Close signal` as isolated event | related `SignalPlan` | Close/reduce signals belong to the accepted opening SignalPlan lifecycle |
| `Position` as broker-only snapshot | account-owned position truth | Account must explain why, when, how, and current state for each position |
...
- `deployment_service`
- `market_data_service_catalog`
- `market_data_pipeline_registry`
- `broker_sync_service`
...
- `market_data_pipeline_selected`
- `market_data_pipeline_stale`
- `broker_sync_stale`
...
**Intent:** Stop new position-opening orders for that deployment.

**Should:**
- block new opens for that deployment
- cancel resting opening orders attributed to that deployment that are not backing existing positions
...
- MarketDataServiceRecord configures an external data provider.
- MarketDataPipeline delivers shared market data.
- Broker account event streams are account-scoped and must reconcile through BrokerSync.
- Feature Engine must never call external providers.
- BrokerAdapter must not be used by Chart Lab, Sim Lab, Backtest, or Live Preview.
- Real broker ledgers and simulated ledgers must remain separate.
...
MarketDataPipelines deliver shared data, and the Broker Account is the external
source of truth.
