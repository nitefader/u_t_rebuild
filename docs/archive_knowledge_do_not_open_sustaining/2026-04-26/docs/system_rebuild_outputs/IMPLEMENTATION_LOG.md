...
## 2026-04-26 17:25 ET - SignalPlan Lifecycle Addendum Adopted

Task:
- Add the operator-approved SignalPlan lifecycle and account-owned position explanation rule.

Files changed:
- Simplified Runtime Architecture and Guiding Principles.md
- docs/Canonical_Architecture.md
- final_roadmap_and_arch_decisions_and_guidelines.md
- memory/authority_docs.md
- memory/MEMORY.md
- docs/system_rebuild_outputs/IMPLEMENTATION_LOG.md

Implemented:
- Added the April 26 operator addendum to the simplified architecture.
- Locked the rule that an Account accepting an opening SignalPlan must keep processing related close, reduce, scale-out, stop, target, runner, and logical-exit SignalPlans from the same Deployment for the resulting position.
- Clarified that close SignalPlans can be partial, such as close 50 percent, and must not imply automatic full flatten.
- Added Account-owned position explanation requirements: signal lineage, account decision trace, Governor decision, orders, fills, current risk, and related close/reduce SignalPlans.
- Added the "Explain this position" AI action as advisory-only context usage.

Scope kept out:
- No runtime schema/code migration yet.
- No UI implementation yet.
- No AI service implementation yet.
...
## 2026-04-26 17:04 ET - Architecture and Naming Contract Tightening

Task:
- Clarify the canonical architecture and naming contract before the next runtime slice.

Files changed:
- docs/Canonical_Architecture.md
- docs/Services_Architecture_Market_Data_AI.md
- docs/architecture/services_architecture.md
- final_roadmap_and_arch_decisions_and_guidelines.md
- docs/system_rebuild_outputs/IMPLEMENTATION_LOG.md

Implemented:
- Added current implementation alignment anchors for TradingMode, BrokerAccount, Deployment, MarketDataServiceRecord, MarketDataPipeline, PortfolioGovernor, OrderManager, BrokerAdapter, and BrokerSync.
- Added the full data-and-execution spine and the architecture rule: Features drive demand; Pipelines deliver data; Accounts isolate risk; Governors approve trades; BrokerSync writes broker truth.
- Defined MarketDataServiceRecord, MarketDataPipeline, and Broker Account Event Stream as separate concepts.
- Clarified Providers page naming, Broker Runtime - Paper / Broker Runtime - Live labels, and Deployment Pause instead of Program Pause.
- Updated service architecture docs so services are provider records and runtime consumers attach to Market Data Pipelines.

Scope kept out:
- No code migration of legacy tool names such as run_paper_runtime.
- No route or UI rename beyond documentation.
- No full backend/frontend test suite run.

Validation performed:
- python -m pytest backend\tests\unit\domain\test_domain_boundaries.py backend\tests\unit\market_data\test_pipeline_registry.py backend\tests\unit\lint\test_no_banned_mode_enums.py -q

Result:
- 176 passed, 1 warning.

Verification:
- TradingMode boundary tests remain green.
- MarketDataPipeline rejects non-broker TradingMode bindings and preserves active pipeline identity invariants.
- Banned bare PAPER/LIVE mode enum lint remains green.
...
## 2026-04-26 17:15 ET - Simplified Runtime Guidance Adopted

Task:
- Promote the latest simplified runtime architecture guidance to the first authority.
...
Implemented:
- Updated authority read order so `Simplified Runtime Architecture and Guiding Principles.md` overrides older Program-centric guidance.
- Added a canonical override summary: no user-facing Program in V1; Deployment publishes SignalPlans; Accounts subscribe and independently decide, size, and execute.
- Marked Account risk config and Strategy execution config as the simplified V1 path instead of mandatory separate Risk Profile / Execution Style tables.
- Preserved the roadmap as binding for phase order, resolver contract, stop conditions, validation, logging, and engineering defaults where it does not conflict.

Scope kept out:
- No data model migration yet.
- No route/UI rename yet.
- No runtime code changes yet.

Validation performed:
- Documentation/authority update only; no test run after this note.

Result:
- Pending implementation validation in the next code slice.
