...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run one controlled paper runtime pass.")
    ...
    load_dotenv()
    _print_step("Validating paper runtime guards")
    ...
    _print_step("Creating paper Deployment context")
...
def _validate_environment(*, bars: int, qty: int) -> str | None:
    ...
    if bars > 20:
        return "bars > 20 is blocked by controlled paper runtime"
    if qty <= 0:
        return "qty must be greater than 0"
    if qty > 1:
        return "qty > 1 is blocked by controlled paper runtime"
...
def _components(*, symbol: str, qty: int) -> ResolvedProgramComponents:
    strategy_id = uuid4()
    controls_id = uuid4()
    risk_id = uuid4()
    execution_id = uuid4()
    ...
    strategy = StrategyVersion(
        ...
        strategy_id=uuid4(),
        version=1,
        name="Paper Runtime Smoke Strategy",
        entry_rules=[
            SignalRule(
    ...
    risk = RiskProfileVersion(
        ...
        version=1,
        name="Paper Runtime Fixed Shares",
...
def _print_step(message: str) -> None:
    print(f"[run_paper_runtime] {message}", flush=True)
