...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run a controlled account runtime dry-run from Alpaca market data.")
    ...
    load_dotenv()
    _print_step("Validating account runtime dry-run guards")
...
def _validate_environment(*, bars: int, qty: int) -> str | None:
    ...
    if bars > 5:
        return "bars > 5 is blocked by account runtime dry-run"
    if qty <= 0:
        return "qty must be greater than 0"
    if qty > 1:
        return "qty > 1 is blocked by account runtime dry-run"
...
def _components(*, symbol: str, timeframe: str, qty: int) -> ResolvedDeploymentComponents:
    strategy_id = uuid4()
    controls_id = uuid4()
    risk_id = uuid4()
    execution_id = uuid4()
    ...
    risk = RiskProfileVersion(
        ...
        version=1,
        name="Runtime Dry Run Fixed Shares",
    ...
    universe = UniverseSnapshot(
        ...
        universe_id=uuid4(),
        version=1,
        name="Runtime Dry Run Universe",
        symbols=[UniverseSymbol(symbol=symbol)],
    )
    program = ProgramVersion(
        id=uuid4(),
        program_id=uuid4(),
        name="Runtime Dry Run Strategy Version",
        version=1,
...
def _print_step(message: str) -> None:
    print(f"[run_runtime_dry_run] {message}", flush=True)
